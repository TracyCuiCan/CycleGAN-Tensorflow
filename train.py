...
parser.add_argument('--load_size', dest='load_size', type=int, default=286, help='scale images to size')
...
d_a_train = tf.train.AdamOptimizer(lr, beta1=0.5).minimize(d_loss_a, var_list=d_a_var)
d_b_train = tf.train.AdamOptimizer(lr, beta1=0.5).minimize(d_loss_b, var_list=d_b_var)
g_train = tf.train.AdamOptimizer(lr, beta1=0.5).minimize(g_loss, var_list=g_var)
...
ckpt_path = util.load_checkpoint(ckpt_dir, sess, saver)
...
try:
	...
	batch_epoch = min(len(trainA_pool), len(trainB_pool)) // batch_size
	max_it = epoch * batch_epoch

	for it in range(sess.run(cnt), max_it):
		sess.run(update_cnt)

		#Prepare data
		realA_ipt = trainA_pool.batch()
		realB_ipt = trainB_pool.batch()
		a2b_opt, b2a_opt = sess.run([a2b, b2a], feed_dict={a_real: realA_ipt, b_real: realB_ipt})
		a2b_sample_ipt = np.array(a2b_pool.query(list(a2b_opt)))
		b2a_sample_ipt = np.array(b2a_pool.query(list(b2a_opt)))

		#train G
		g_summary_opt, _ = sess.run([g_summary, g_train], feed_dict={a_real: realA_ipt, b_real: realB_ipt})
		summary_writer.add_summary(g_summary_opt, it)

		#train D_b
		d_summary_b_opt, _ = sess.run([d_summary_b, d_b_train], feed_dict={b_real: realB_ipt, a2b_sample: a2b_sample_ipt})
		summary_writer.add_summary(d_summary_b_opt, it)

		#train D_a 
		d_summary_a_opt, _ = sess.run([d_summary_a, d_a_train], feed_dict={a_real: realA_ipt, b2a_sample: b2a_sample_ipt})
		...
		#sample
		if (it + 1) % 100 == 0:
			realA_ipt = testA_pool.batch()
			realB_ipt = testB_pool.batch()
			[a2b_opt, a2b2a_opt, b2a_opt, b2a2b_opt] = sess.run([a2b, a2b2a, b2a, b2a2b], feed_dict={a_real: realA_ipt, b_real: realB_ipt})
			sample_opt = np.concatenate((realA_ipt, a2b_opt, a2b2a_opt, realB_ipt, b2a_opt, b2a2b_opt), axis=0)
